define(function(require,exports,module){
	class Public{
		...
		successHtml(con){
			var str;
			str+='<div id="Winpop">'
			+'<div id="layerBox" class="h340"><div class="successBox"><img class="img-width-success" src="https://static.liantuobank.com/project/lianfutong/images/right_ico.png" style="width:12%;" title=""></div>'+con+'</div>'
			+'<div id="popBtn" class="g-tc">'
			+'<div class="bluebtn save">'
			+'<a class="btn wBox_close">关闭</a>'
			+'</div>'
			+'</div>'
			+'</div>'
			return str;
		...
		checkboxList(obj){
			...
			$(document).on("click",".allChk",function(){
				$("#"+obj).val("");
				var arr=[],flag=($(this)[0].checked),children=$(".singleChk");
		...
		allcheck(object,object2){//传赋值隐藏域的id
			//全选和反选
			$(document).on("click",".tablecs input",function(){
				var obj=$(".table .uicheckbox"),str="",strnum="";
		...
		gettabWidth(){
			$(".tabBox .tcol").each(function(){
			   ...
			   var _parentWidth = $(this).parent("td").width(),str=$(this).attr("title"),str1="";
		...
		setparentTabwidth(){ //寻找父级元素，设置table宽度
